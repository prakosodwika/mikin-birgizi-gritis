...
const emptyBahan = (): BahanForm => ({
    nama_bahan: '', satuan: 'gram', jumlah: 0,
    kalori_per_100g: 0, protein_per_100g: 0,
    lemak_per_100g: 0, karbohidrat_per_100g: 0, harga_per_satuan: 0,
});

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Program Gizi', href: '/pengelola/program-gizi' },
    { title: 'Tambah Program', href: '#' },
];
...
export default function ProgramGiziCreate({ standar_gizi, prefill_date }: Props) {
    const [bahans, setBahans] = useState<BahanForm[]>([emptyBahan()]);
    ...
    const addBahan = () => {
        const updated = [...bahans, emptyBahan()];
        setBahans(updated);
        setData('bahans', updated);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(programGiziStore().url);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Tambah Program Gizi" />
            <form onSubmit={handleSubmit}>
                <div className="flex flex-col gap-6 p-6">
                    {/* Header */}
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Button type="button" variant="ghost" size="icon" onClick={() => router.visit(programGiziIndex().url)}>
                                <ArrowLeft className="size-4" />
                            </Button>
                            <div>
                                <h1 className="text-2xl font-semibold">Tambah Program Gizi</h1>
                                <p className="text-sm text-muted-foreground">Sistem akan otomatis memvalidasi gizi terhadap standar BGN saat disimpan.</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <ValidationBadge status={validationStatus} />
                            <Button type="button" variant="outline" onClick={() => router.visit(programGiziIndex().url)}>Batal</Button>
                            <Button type="submit" disabled={processing}>Simpan Program</Button>
                        </div>
                    </div>
                        ...
                        <div className="col-span-2 space-y-6">
                            ...
                            <Card>
                                ...
                                <CardContent className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="nama_program">Nama Program <span className="text-destructive">*</span></Label>
                                        <Input id="nama_program" value={data.nama_program} onChange={e => setData('nama_program', e.target.value)} placeholder="cth. Nasi Ayam Sayur Bayam" />
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="jumlah_porsi">Jumlah Porsi (batch) <span className="text-destructive">*</span></Label>
                                        <Input id="jumlah_porsi" type="number" min={1} value={data.jumlah_porsi} onChange={e => setData('jumlah_porsi', parseInt(e.target.value))} />
                                    </div>
                                    <div className="col-span-2 space-y-2">
                                        <Label htmlFor="catatan">Catatan (Opsional)</Label>
                                        <Textarea id="catatan" value={data.catatan} onChange={e => setData('catatan', e.target.value)} placeholder="Tambahkan catatan atau keterangan tambahan..." rows={2} />
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                ...
                                <CardContent className="p-0">
                                            ...
                                            {bahans.map((bahan, idx) => (
                                                <TableRow key={idx}>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" value={bahan.nama_bahan} onChange={e => updateBahan(idx, 'nama_bahan', e.target.value)} placeholder="cth. Beras" />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.jumlah} onChange={e => updateBahan(idx, 'jumlah', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" value={bahan.satuan} onChange={e => updateBahan(idx, 'satuan', e.target.value)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.kalori_per_100g} onChange={e => updateBahan(idx, 'kalori_per_100g', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.protein_per_100g} onChange={e => updateBahan(idx, 'protein_per_100g', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.lemak_per_100g} onChange={e => updateBahan(idx, 'lemak_per_100g', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.karbohidrat_per_100g} onChange={e => updateBahan(idx, 'karbohidrat_per_100g', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                                        <Input className="h-8 text-sm" type="number" min={0} value={bahan.harga_per_satuan} onChange={e => updateBahan(idx, 'harga_per_satuan', parseFloat(e.target.value) || 0)} />
                                                    </TableCell>
                                                    <TableCell className="p-1.5">
                                    ...
                                    {errors.bahans && <p className="text-xs text-destructive px-4 py-2">{errors.bahans}</p>}
                                </CardContent>
                            </Card>
                        </div>
                        ...
                        <div className="space-y-4">
                            <Card className="sticky top-6">
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {/* Nutritional totals vs standard */}
                                        ...
                                        return (
                                            <div key={key} className="space-y-1">
                                                ...
                                                <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full transition-all ${ok === true ? 'bg-green-500' : ok === false ? 'bg-red-400' : 'bg-primary'}`}
                                                        style={{ width: `${pct}%` }}
                                                    />
                                                </div>
                                            </div>
